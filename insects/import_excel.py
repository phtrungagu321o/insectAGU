                        ...
                        if len(order) > 0:
                            ...
                            if len(family) > 0:
                                ...
                                if len(genus) > 0:
                                    ...
                                    if len(species) > 0:
                                        self.res[i] = False
                                        self.warningMessage += "<div class='alert alert-danger'> loài: <strong>" + \
                                                               self.species[i] + "</strong> Đã tồn tại</div>"
                                    else:
                                        tmp = models.InsectTest.objects.filter(eName=self.eName[i])
                                        if len(tmp) > 0:
                                            for index in range(len(tmp)):
                                                self.warningMessage += "<div class='alert alert-warning'>loài: " + \
                                                                       self.species[i] + " tồn tại trong <b>" + tmp[
                                                                           index].genus.eName + "</b> : <b> " + \
                                                                       self.genus[i] + " </b> index: " + str(
                                                    i + 1) + " </div>"
                                                self.res[i] = False
                                        else:
                                    ...
                                    if len(tmp) > 0:
                                        for index in range(len(tmp)):
                                            ...
                                            if tmp[index].family.eName != self.family[i]:
                                                ...
                                                print("~~~~~~~~~~~~~~~~~~~~~~~~")
                                                self.warningMessage += "<div class='alert alert-warning'>genus: " + \
                                                                       self.genus[i] + " tồn tại trong <b>" + tmp[
                                                                           index].family.eName + "</b> : <b> " + \
                                                                       self.family[i] + " </b> index: " + str(
                                                    i + 1) + " </div>"
                                    else:
                                ...
                                if len(tmp) > 0:
                                    for index in range(len(tmp)):
                                        ...
                                        if tmp[index].order.eName != self.order[i]:
                                            ...
                                            print("~~~~~~~~~~~~~~~~~~~~~~~~")
                                            self.warningMessage += "<div class='alert alert-warning'>family: " + \
                                                                   self.family[i] + " tồn tại trong <b>" + tmp[
                                                                       index].order.eName + "</b> : <b> " + self.order[
                                                                       i] + " </b> index: " + str(i + 1) + " </div>"
                                else:
                            ...
                            if len(tmp) > 0:
                                for index in range(len(tmp)):
                                    ...
                                    if tmp[index].classes.eName != self._class[i]:
                                        self.warningMessage += "<div class='alert alert-warning'>order: " + self.order[
                                            i] + " tồn tại trong <b>" + tmp[index].classes.eName + "</b> : <b> " + \
                                                               self._class[i] + " </b> index: " + str(i + 1) + " </div>"
                            else:
                        ...
                        if len(tmp) > 0:
                            for index in range(len(tmp)):
                                ...
                                if tmp[index].phylum.eName != self.phylum[i]:
                                    self.warningMessage += "<div class='alert alert-warning'>class: " + self._class[
                                        i] + " tồn tại trong <b>" + tmp[index].phylum.eName + "</b> : <b> " + \
                                                           self.phylum[i] + " </b> index: " + str(i + 1) + " </div>"
                        else:
        ...
        for i in range(len(self.name)):
            ...
            if self.res[i] != False:
                if self.existPhylum[i] == False:
                    ...
                    phylum = models.Phylum(kingdom=kingdom, eName=self.phylum[i], name=self.phylum[i],
                                           slug="phylum_" + self.phylum[i].replace(" ", "_"))
                    phylum.save()

                if self.existClass[i] == False:
                    phylum = models.Phylum.objects.filter(eName=self.phylum[i])[0]
                    _class = models.Classes(phylum=phylum, eName=self._class[i], name=self._class[i],
                                            slug="class_" + self._class[i].replace(" ", "_"))
                    _class.save()

                if self.existOrder[i] == False:
                    _class = models.Classes.objects.filter(eName=self._class[i])[0]
                    order = models.Order(classes=_class, eName=self.order[i], name=self.order[i],
                                         slug="order_" + self.order[i].replace(" ", "_"))
                    order.save()

                if self.existFamily[i] == False:
                    order = models.Order.objects.filter(eName=self.order[i])[0]
                    family = models.Family(order=order, eName=self.family[i], name=self.family[i],
                                           slug="family_" + self.family[i].replace(" ", "_"))
                    family.save()

                if self.existGenus[i] == False:
                    family = models.Family.objects.filter(eName=self.family[i])[0]
                    genus = models.Genus(family=family, eName=self.genus[i], name=self.genus[i],
                                         slug="genus_" + self.genus[i].replace(" ", "_"))
                    genus.save()

                if self.existSpecies[i] == False:
                    genus = models.Genus.objects.filter(eName=self.genus[i])[0]
                    insect = models.Insect(genus=genus, eName=self.eName[i], name=self.name[i],
                                           slug="insect_" + self.eName[i].replace(" ", "_"),
                                           characteristic=self.characteristic[i], value=self.value[i],
                                           reality=self.reality[i], protective=self.protective[i],
                                           distribution=self.distribution[i])
                    insect.save()
                    self.warningMessage += "<div class='alert alert-success'><b>" + self.eName[
                        i] + "</b>  Save succeed</div>"
