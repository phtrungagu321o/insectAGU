...
def index_map1(request):
    ...
    df = pd.read_csv('data.csv')
    if request.method == "POST":
        searched = request.POST['search_insect']
        m = folium.Map(location=[19, -12], tiles="Stamen Terrain", width=1000, zoom_start=2)
        for i, row in df.iterrows():
            lat = df.at[i, 'lat']
            lng = df.at[i, 'lng']
...
def search_img(request):
    if request.method == 'POST':
        ...
        try:
            ...
            label = parsed[0]['name']
            print(label)
            label_complete = label.replace(' ', '_')
            path_save = str('assets/img/exp/' + label_complete)
            results.save(save_dir=path_save)
            path_save1 = str('img/exp/' + label_complete + '/' + file_names)
            return render(request, 'insect/search_tool.html', {
                'ms': '',
                'path_ins': path_save1,
                'name_sc': name_sce,
            ...
            return render(request, 'insect/search_tool.html', {'ms': 'Thất bại', 'path_ins': '',
                                                               'name_sc': ''})
    return render(request, 'insect/search_tool.html', {'ms': 'Thất bại', 'path_ins': '', 'name_sc': ''})


from matplotlib import pyplot
from numpy.random import randint

def insect_multilien(request):
    fig, (a1, a2) = pyplot.subplots(1, 2, figsize=(10, 4), sharey=True, dpi=120)
    data = pd.read_csv("salaries.csv")
    ages = data['Age']
    Rice_leaf_roller = data['Cnaphalocrocis_medinalis']
    Rice_leaf_caterpillar = data['Chlorops_oryzae']
    # java_salaries = data["Java"]
    # python_salaries = data['Python']
    # csharp_salaries = data['C#']
    a1.plot(ages, Rice_leaf_roller,
            color='#04DE71',
            linestyle='--',
            label="Cnaphalocrocis_medinalis"

            )
    a2.plot(ages, Rice_leaf_caterpillar,
            color='#00F5EA',
            label="Chlorops_oryzae"
            )
    # a3.plot(ages, python_salaries,
    #             color='pink',
    #             label="Python dev"
    #             )
    # a4.plot(ages, csharp_salaries,
    #             color="#2094FA",
    #             label="C#"
    #             )
    pyplot.tight_layout()
    pyplot.show()
def insect_multilien1(request):
    if request.method == "POST":
        searched = request.POST['search_insect1']
        print(searched)
        data = pd.read_csv("salaries.csv")
        ages = data['Age']
        name = data[searched]
